...
export default function ColorMixer() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-12">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-white mb-2">Color Mixer</h1>
        <p className="text-gray-400 text-sm">Blend two colors together and see the result at different ratios.</p>
      </div>

      <div className="grid sm:grid-cols-2 gap-4 mb-6">
        {[
          { label: 'Color A', value: c1, set: setC1, valid: v1 },
          { label: 'Color B', value: c2, set: setC2, valid: v2 },
        ].map(col => (
      ...
      <div className="card p-5 mb-6">
        <div className="flex items-center justify-between mb-3">
          <label className="label mb-0">Mix Ratio</label>
      ...
      <div className="card p-5">
        <h2 className="font-semibold text-white mb-4">All Mix Steps</h2>
        <div className="grid grid-cols-11 gap-1.5">
          {steps.map(step => {
            const m = mixColors(v1, v2, step / 100);
            return (
              <div key={step} className="group cursor-pointer" onClick={() => copy(m?.toUpperCase())}>
                <div className="w-full aspect-square rounded-lg border border-dark-600 group-hover:scale-110 transition-transform"
                  style={{ background: m }} />
        ...
        <p className="text-xs text-gray-600 mt-3 text-center">Click any swatch to copy its HEX code</p>
      </div>
    </div>
  );
}
